    ...
    return (
      <div>
        {options.map((option) => {
          return (
            <div key={option.value}  style={{ display: "flex", alignItems: "center", padding: "8px" }}>
              ...
              checked={value.includes(option.value)} className="custom-checkbox">              
                {option.label}
              </Checkbox>
            </div>
          );
        ...
        <div  style={{ display: "flex", alignItems: "center", padding: "8px" }}>
            <Checkbox indeterminate={indeterminate} onChange={onCheckAllChange} checked={checkAll} className="custom-checkbox">
              {allLabel}
          </Checkbox>
        </div>
        
      <div style={{ display: "flex", padding: "8px" }}>
          <Button onClick={handleReset} className="btn-1">Reset</Button>
          <Button onClick = {handleOk} className="btn-2">OK</Button>
      </div>
      </div>
    );
  }
